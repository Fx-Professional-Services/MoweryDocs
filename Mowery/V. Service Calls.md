...
3. Unassigned HVAC calls appear in peach in the left column.  
...
### Specify a Type for Each Category

1. Click **Service Calls → System Defaults → Category**.  
2. Click **New Item**.  
...
> The **Type** field distinguishes categories with the same name.  
...
The type displays in selection lists to avoid confusion.

### Assign Subcategories to Categories

1. Click **Service Calls → System Defaults → Sub Category**.  
2. Select a subcategory.  
3. Choose the parent category from the dropdown.  

> Subcategories inherit their parent category's type.  

### Assign Products to a Subcategory

All products require a subcategory. If no specific subcategory applies, assign to:  
- `Miscellaneous (HVAC)`  
- `Miscellaneous (Plumbing)`  
